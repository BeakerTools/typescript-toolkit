...
import {ProgrammaticScryptoSborValue, ProgrammaticScryptoSborValueMapEntry} from "@radixdlt/babylon-gateway-api-sdk";
...
export function parseNonFungibleData(data: ProgrammaticScryptoSborValue): NonFungibleData {
  let name = data.field_name? data.field_name : "";
  switch (data.kind){
    case "Array":{
      return {name: name, value: "(" + dataArrayToString(data.elements) + ")"};
    }
    case "Bool":{
    ...
    case "Map":{
      ...
      if(data.key_type_name && data.value_type_name){
        value = "<" + data.key_type_name +", "+data.value_type_name+">" + value
      }
      value = "Map"+value
